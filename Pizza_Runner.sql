...
SELECT c.customer_id,
       AVG(CAST(REPLACE(r.distance,'km','') AS DECIMAL(5,2))) AS avg_distance_km
...
SELECT runner_id,
       100.0 * SUM(CASE WHEN distance IS NOT NULL THEN 1 ELSE 0 END) / COUNT(*) AS success_pct
FROM runner_orders
GROUP BY runner_id;
...
SELECT p.pizza_name,
       GROUP_CONCAT(t.topping_name ORDER BY t.topping_name) AS ingredients
